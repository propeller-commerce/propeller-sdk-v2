...
export class PurchaseAuthorizationConfigResponse {
  ...
  /** Returns `items` as PurchaseAuthorizationConfig instances (coerced on first access). */
  getItems(): PurchaseAuthorizationConfig[] {
    if (!this.items) return [];
    this.items = this.items.map((x: any) => x instanceof PurchaseAuthorizationConfig ? x : new PurchaseAuthorizationConfig(x));
    return this.items;
  }
  /** Returns `itemsFound`. */
