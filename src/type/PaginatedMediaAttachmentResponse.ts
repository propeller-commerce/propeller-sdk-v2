...
export class PaginatedMediaAttachmentResponse {
  ...
  /** Returns `items` as MediaAttachment instances (coerced on first access). */
  getItems(): MediaAttachment[] {
    if (!this.items) return [];
    this.items = this.items.map((x: any) => x instanceof MediaAttachment ? x : new MediaAttachment(x));
    return this.items;
  }
  /** Returns `itemsFound`. */
