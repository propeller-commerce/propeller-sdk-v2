...
export class CartMainItem implements ICartBaseItem {
  ...
  /** Returns `bundle` as a Bundle instance (coerced on first access). */
  getBundle(): Bundle | undefined {
    if (this.bundle == null) return undefined;
    if (!(this.bundle instanceof Bundle)) {
      this.bundle = new Bundle(this.bundle as any);
    }
    return this.bundle;
  }
  ...
  /** Returns `childItems` as CartBaseItem instances (coerced on first access). */
  getChildItems(): CartBaseItem[] | undefined {
    if (!this.childItems) return undefined;
    this.childItems = this.childItems.map((x: any) => x instanceof CartBaseItem ? x : new CartBaseItem(x));
