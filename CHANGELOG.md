...
attribute shape, so listings can render specs without a second round trip to
fetch each product in full.

Additive and backward-compatible: the new operation variable is optional, and
no service method signature changed.

### Added

- **`AttributeResultGridFields`** — a new fragment on `AttributeResult`
  selecting only `attributeDescription { name }` plus the four value branches
  that listings use: `AttributeTextValue.textValues`,
  `AttributeEnumValue.enumValues`, `AttributeIntValue.intValue` and
  `AttributeDecimalValue.decimalValue`. This is deliberately *not*
  `AttributeResultProductFields`, which pulls the full 18-field description,
  the `attribute` union and all six value types — far too much payload when it
  multiplies by every tile in a grid.
- `attributes (input: $attributeResultSearchInput)` on **`ProductGridFields`**.
- `$attributeResultSearchInput: AttributeResultSearchInput` declared on the
  **21 operations** that transitively spread `ProductGridFields` — the
  `category*` queries and mutations, `products`, `crossupsell(s)`,
  `crossupsellCreate` / `Update`, `favoriteList`, `machine(s)`, `machineCreate`
  and `machineUpsert`. The fragment reaches them through `ClusterGridFields`,
  `CrossupsellFields`, `FavoriteListFields`, `ProductsResponseFields` and
  `SparePartFields`, and GraphQL rejects an operation that spreads a fragment
  using a variable it does not declare, so all 21 are required. The `product`
  query already had it.
- `attributeResultSearchInput` on the three **hand-authored** service variables
  interfaces, which codegen does not regenerate:
  `ProductsQueryVariables` (`ProductService.getProducts`),
  `CategoryQueryVariables` (`CategoryService.getCategory`) and
  `CrossupsellsQueryVariables` (`CrossupsellService.getCrossupsells`). The other
  18 operations use generated interfaces and picked the field up automatically.
  Without this the value reached the wire at runtime — the method bodies spread
  `{ ...variables }` — but TypeScript callers could not pass it without a cast.
  `ProductQueryVariables` already exposed it and is unchanged.
- `tests/service/ProductGridAttributes.integration.test.ts` — real-API tests
  covering the shape, the union branches, and that
  `$attributeResultSearchInput` genuinely reaches the server. They run through
  the service layer with the variables annotated as their real interfaces, so a
  regression in any of those hand-authored types is a compile error rather than
  a silent pass.

### Fixed

- Docs build. Docusaurus 3 compiles `.md` as MDX, where a bare `<` starts a JSX
  tag; the 0.15.0 stock-filtering prose wrote the inclusive operators as
  `"<= N"` and broke both `guides/stock-filtering.md` and the generated
  `changelog.md`. The comparisons are now wrapped in inline code.

### Notes

The corresponding `<Op>Variables` interfaces gained an optional
`attributeResultSearchInput`, so existing calls compile unchanged. Pass it to
control how many attributes come back per product:

```typescript
const grid = await products.getProducts({
  input: { page: 1, offset: 24 },
  attributeResultSearchInput: { page: 1, offset: 25 },
});
```

`AttributeColorValue` and `AttributeDateTimeValue` are intentionally not
selected by the grid fragment — an attribute of either type arrives carrying
only `__typename`. Read those through the full `product` query instead.

---

## [0.15.0] - 2026-08-04

Exposes the backend inventory/stock filter so storefronts can filter catalogue
and category listings by stock. Additive and backward-compatible — the filter
is optional everywhere it appears.

Filtering is applied server-side, so the pagination fields on the response
(`itemsFound`, `pages`) describe the filtered set and can be used directly for
paging.

### Added

- **`ProductSearchInventoryFilterInput`** — new exported type with
  `totalQuantity`, `localQuantity` and `supplierQuantity`, each a
  `NumberSearchInput`.
- Optional `inventory` field on all five product-search inputs that accept it
  upstream: `ProductSearchInput`, `CategoryProductSearchInput`,
  `ProductSearchByCategoryInput`, `ProductSearchSearchInput` and
  `SparePartsMachineProductSearchInput`. No service or operation changes were
  needed — these inputs are already passed through as variables, so the filter
  is reachable from `productService.getProducts({ input })`,
  `productSearchByCategory({ input })` and the `categoryProductSearchInput`
  variable on the category queries.
- `docs/content/guides/stock-filtering.md` and a README section covering the
  operator semantics.
- `tests/service/ProductInventoryFilter.integration.test.ts` — guards that the
  filter is applied server-side and that `lessThan` really is inclusive.

### Notes

`NumberSearchInput`'s range operators are **inclusive** despite their names:
`greaterThan: N` is stock `>= N` and `lessThan: N` is stock `<= N`.
Never-stocked products have no inventory record and count as **0**. So:

- in stock — `{ totalQuantity: { greaterThan: 1 } }` (`>= 1`; correctly includes
  products with exactly 1 left)
- out of stock — `{ totalQuantity: { equal: 0 } }`, or
  `{ totalQuantity: { lessThan: 0 } }` (`<= 0`) to also catch negative balances

Do **not** use `lessThan: 1` for out-of-stock — it means `<= 1` and includes
in-stock products with one item remaining.

The committed schema snapshot is refreshed to the production introspection that
carries this filter.
...
**Breaking.** Removes the entire deprecated surface — every operation, field
selection, type member and enum value the upstream schema marks `@deprecated`,
plus the service methods that wrapped them. This reverses the v0.12.0 decision
to keep deprecated members annotated for a soft migration; there is no longer a
compatibility shim for any of them.

Consumers must migrate before upgrading. Anything calling the removed service
methods, or reading the removed response fields, will fail to compile.

### Removed

- **23 deprecated root operations**, with their `.graphql` documents and
  generated modules (546 → 523 operations):
  - Queries: `addressesByOrderId`, `companySearch`, `mediaDocument`,
    `mediaDocuments`, `mediaImage`, `mediaImages`, `mediaVideo`, `mediaVideos`,
    `shop`, `shops`
  - Mutations: `bundleAddItems`, `bundleRemoveItem`, `carrierAssignWarehouse`,
    `carrierUnassignWarehouse`, `cartSetUser`,
    `companySearchCleanupOrphanedIndices`, `companySearchFixAliasWriteIndex`,
    `companySearchIndexUpdateMapping`, `companySearchReindex`, `logout`,
    `orderUpdateAddress`, `publishPasswordResetEmailEvent`,
    `shopInvalidateCache`
- **`ShopService` and `LogoutService` deleted** — every method they exposed
  wrapped a removed operation. Also dropped from the `src/service` barrel.
- **10 service methods** across 9 services (removed from both the factory
  object and its backwards-compatible class form):
  `AddressService.getAddressesByOrderId`, `OrderService.getAddressesByOrderId`
  (use `getOrderAddresses`), `BundleService.addItemsToBundle` (use
  `addItemsToBundleAndReturnBundle`), `CartService.setCartUser` (use
  `setCartContact` / `setCartCustomer`),
  `EventActionConfigService.publishPasswordResetEmailEvent` (use
  `UserService.sendPasswordResetEmail`), `UserService.logout`, and the
  `getMediaImage(s)` / `getMediaVideo(s)` / `getMediaDocument(s)` readers on the
  three media services (read via `media.image` / `.images` etc. on the parent
  resource).
- **42 deprecated field selections** from 17 fragments — notably
  `Category.name/description/shortDescription/slug/path`,
  `IBaseProduct.class/language`, `Product.class/language`,
  `OrderStatus.isExportable/isConfirmable/isArchivable`,
  `Channel.channelId/defaultLetterId/shop`, `ClusterConfigSetting.id/name`,
  `Contact.debtorId`, `Inventory.dateModified`, `BusinessRule.shopId`, and
  eight fields on `TenderFields`.
- **62 members** from 32 files in `src/type` and `src/enum`, including the
  enum values `SortOrder.asc`/`.desc` (use `ASC`/`DESC`),
  `PriceSortField.COST_PRICE`, `OrderItemClass.postage`/`.payment`, and
  `CategorySortableFields.dateCreated`/`.dateChanged` (use `createdAt` /
  `lastModifiedAt`).

### Fixed

- `CategoryMinimalFields` and `CategoryRelationshipFields` now pass
  `(language: $language)` to `names`, `descriptions`, `shortDescriptions` and
  `slugs`. The deprecated singular fields they replaced accepted that argument
  and the plural forms accept it identically, so without this the fragments had
  silently lost per-language filtering. `categories` keeps its `$language`
  variable and `CategoriesVariables` is unchanged.
- Tagged five members the schema had newly deprecated and the SDK had not
  caught, then removed them with the rest: `PriceCreateInput.cost`,
  `PriceSearchInput.cost`, `PriceUpdateInput.cost`, `TaxCreateInput.shopId`,
  `TaxSearchInput.shopId`.

### Changed

- `scripts/.schema-drift-exceptions.json` reseeded. The
  `allowedDeprecatedSelections` category is now **empty** (was 737 entries
  across 137 operations) and `exemptFields` drops from 508 to 459 — the 18
  "SDK still declares a member the schema marks `@deprecated`" exemptions are
  gone. Total unfiltered drift 1492 → 513.
- `tests/service/LogoutService.test.ts` replaced by
  `tests/service/TenantService.test.ts`; `tenant` is likewise a zero-variable
  operation, so the zero-variable service contract stays covered.

### Notes

- The three `@deprecated` annotations in `src/client/GraphQLClient.ts` are
  **retained** — they mark SDK-level API choices (`createClient()` over the
  constructor, and the global-singleton pattern), not schema deprecations.
- Schema realigned to the 2026-07-28 production introspection, which also adds
  the `AttributeValueType` object and `CartSearchInput.references`. Neither is
  surfaced yet.
...
Makes the machine / spare-parts surface usable by a storefront. Previously
`machine` fetched its `sparePartProducts` with no argument, so a machine page
could not filter, sort, page or search its parts — the schema supported it, the
shipped document didn't.

### Added

- `sparePartsMachineProductSearchInput: SparePartsMachineProductSearchInput`
  variable on the `machine` and `machines` queries and the `machineCreate` /
  `machineUpsert` mutations (every document that spreads `SparePartsMachineFields`
  must declare it), applied as
  `sparePartProducts(input: $sparePartsMachineProductSearchInput)`. Mirrors how
  `CategoryFields` takes `$categoryProductSearchInput`.
  Note the input is all-or-nothing: `language`, `page`, `offset` and `statuses`
  are non-null on the schema, so any caller passing it must pass those four.
- `filterAvailableAttributeInput: FilterAvailableAttributeInput` variable on the
  same four documents, applied as `filters(input: $filterAvailableAttributeInput)`
  in `SparePartsResponseFields` — mirrors `ProductsResponseFields`. Without it a
  machine's parts facets could not be requested, so a filter sidebar over spare
  parts would render empty.
- `fetchOptions?: GraphQLFetchOptions` second parameter on
  `machineService.getMachine` / `.getMachines` and their
  `sparePartsMachineService` twins (plus both class shims), matching
  `categoryService.getCategory`. Without it these methods could not carry a
  cache hint, so machine pages had no way to opt into the data cache.
- `SparePartsMachineMinimalFields` fragment — the machine analogue of
  `CategoryMinimalFields`, used for nested/child machines.

### Changed

- `SparePartsMachineFields` now composes `SparePartsMachineMinimalFields` and
  reuses the existing `SparePartsResponseFields` instead of inlining an
  identical selection twice.
- Localized `name` / `description` / `slug` on machine selections now take
  `(language: $language)`, matching `CategoryMinimalFields`. Selections for an
  untranslated machine come back empty rather than as every language.
- Nested child machines no longer select `sparePartProducts`. Child nodes render
  as cards (`id`/`name`/`slug`/`media`); previously every node dragged its whole
  parts list along on every fetch.

### Removed

- Unused `SearchSparePartsMachineInput` / `CreateSparePartsMachineInput` /
  `UpsertSparePartsMachineInput` imports from both machine services.
...
- `hidden` field to the `CategoryMinimalFields` and `CategoryRelationshipFields`
  fragments, so category selections (including `Product`/`Cluster.categoryRelations`)
...
Large schema-alignment release. Re-syncs the hand-authored types, enums,
fragments and services against the current Propeller v2 GraphQL schema. The
dominant theme is a catalog localization + entity-identity overhaul (plural
localized arrays, `uuid`/`createdAt`/`lastModifiedAt`), but the change spans the
whole schema surface — Order, Shipment, Tenant, Company/Contact/Customer/Valueset
search inputs, and more. **Additive and backward-compatible**: deprecated members
are kept (tagged `@deprecated`), nothing was removed.

### Added

- **87 new operations** (25 queries + 62 mutations) with matching service methods:
  - **3 new services** — `ClusterOptionService`, `AgentService`, `ApiKeyService`
    (available as factory functions and backwards-compat classes).
  - **CategoryService** — `getCategoryBySlug`, `getCategoryBySource`,
    `getCategoryChildren`, `getCategoryHierarchy`, `categoryMoveAbove`/`Below`.
  - **ClusterService** — `getClusters`, cluster↔category assignment / bulk
    add-remove, `clusterAssignProduct`/`clusterUnassignProduct`, `clusterSetDefaultCategory`.
  - **ClusterConfigService** — `getClusterConfigSetting`, `getClusterConfigsPaginated`,
    `clusterConfigSettingCreate`/`Update`/`Delete`, `clusterConfigUpdate`.
  - **ProductService** — `getBaseProducts`, `getProductsRaw`, `getProductSearchByCategory`,
    `getProductSearchSolr`, product↔category/cluster placement + reorder + bulk ops,
    and the product-search reindex / batch-metrics admin operations.
- **New localized + identity fields** across catalog types: plural
  `names`/`descriptions`/`shortDescriptions`/`slugs` (`LocalizedString[]`),
  `uuid`, `type`, `categoryIds`, `createdAt`, `lastModifiedAt`, `sortOrder`,
  Product dimensions (`width`/`height`/`depth`/`weight`/`classId`),
  `categoryRelations`, `crossUpsells`, and the new `BaseProductCategoryRelationship`
  and `BaseProductsResponse` types.
- **New fields on other entities** — `Order.company`, `Shipment.sources`,
  `Tenant.defaultAppLanguage`/`defaultDataLanguage`, `OrderItemUpdateInput.supplier`,
  `term`/`termFields`/`termLanguages` on Company/Contact/Customer/Valueset search
  inputs (with new `CompanyTermField`/`ContactTermField`/`CustomerTermField`/
  `ValuesetTermField`/`ValuesetItemTermField` enums), date-range + `uuids`/`slugs`/
  `categoryIds` filters on category search inputs, `CostPrice*Input.validFrom`/`validTo`,
  and new enum values (`CategoryFieldName.SLUG`, `CostPriceSortField.VALID_FROM`/`VALID_TO`,
  `CustomerSortableField.FIRST_NAME`/`LAST_NAME`, `CategorySortableFields` additions).
- **Real-API integration test suite** — per-service tests that exercise the new
  fields/operations against a live endpoint, gated on `.env.local` credentials
  (skip cleanly in CI). `dotenv` now loads `.env.local` in the test setup.

### Deprecated

- The scalar catalog fields superseded by the plural localized arrays are now
  tagged `@deprecated` but **retained** for backward compatibility: `Category.name`/
  `description`/`shortDescription`/`slug`/`path`, `IBaseProduct`/`Product`/`Cluster`
  `.language`/`.class`, `Cluster.sku`/`defaultLanguage`, `Product.defaultLanguage`/`offers`,
  `ClusterConfigSetting.id`/`name`, `AttributeDescription.defaultValue`. Migrate to
  the plural / new fields (`names`, `slugs`, `type`, `uuid`, …); a future major will remove them.

### Changed

- The upstream schema dropped the `userId` argument from the `category` and
  `categories` queries; the SDK operations no longer send it. The service method
  signatures keep the optional `userId` parameter for source compatibility (it is
  now ignored). No other existing operation arguments changed.
- Regenerated the committed CI schema snapshot (`tests/integration/schema.snapshot.json`)
  and re-baselined the schema-drift ledger against the refreshed schema.

## [0.11.4] - 2026-07-02

### Added

- Expanded the `priceData` selection in the `ProductFields` and `ProductGridFields`
  fragments to include `per`, `list`, `costPrices`, `suggested`, `store`,
  `bulkPriceDiscountType` and `defaultTaxCode` (previously only `display` was fetched).
- Added the now-available `validFrom` and `validTo` fields to the `CostPrice` type,
  matching the schema. Additive and backward-compatible.
