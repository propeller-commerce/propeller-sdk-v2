...
Product searches take an `inventory` filter, applied server-side — so
`itemsFound` and `pages` describe the filtered set and can be used directly for
paging:

```typescript
const inStock = await products.getProducts({
  input: { page: 1, offset: 24, inventory: { totalQuantity: { greaterThan: 1 } } },
});
```

`greaterThan` and `lessThan` are **inclusive** (`greaterThan: N` is stock >= N,
`lessThan: N` is stock <= N), and never-stocked products count as 0. So in stock
is `greaterThan: 1` and out of stock is `equal: 0` (or `lessThan: 0` to include
negative balances) — **not** `lessThan: 1`, which means <= 1 and would include
products with one item left. Full detail in the
[stock filtering guide](https://propeller-commerce.github.io/propeller-sdk-v2/guides/stock-filtering).
